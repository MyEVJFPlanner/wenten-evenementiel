...
export default async function handler(req, res) {
  ...
  const { nom, email, telephone, typeEvenement, date, message } = req.body || {};

  if (!nom || !email || !telephone || !typeEvenement) {
...
<div style="font-family:'Inter',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #E2EFEC;">
  <div style="background:#0E8C8C;padding:24px 28px;">
    <p style="color:#fff;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;margin:0 0 6px;">Wenten Événementiel — Nouvelle demande de devis</p>
    <h1 style="color:#fff;font-family:Georgia,serif;font-size:24px;font-weight:400;margin:0;">📋 ${typeEvenement}</h1>
  </div>
  <div style="padding:28px;">
    <table style="border-collapse:collapse;width:100%;border-radius:8px;overflow:hidden;border:1px solid #E2EFEC;">
      ${row("Type d'événement", `<strong>${typeEvenement}</strong>`, false)}
      ${row("Nom", nom, true)}
      ${row("Email", `<a href="mailto:${email}" style="color:#0E8C8C;">${email}</a>`, false)}
      ${row("Téléphone", telephone, true)}
      ${row("Date souhaitée", dateFormatted, false)}
...
<div style="font-family:'Inter',Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;padding:0;border-radius:16px;overflow:hidden;border:1px solid #E2EFEC;">
  ...
  <div style="padding:32px;">
    ...
    <p style="color:#5C7370;line-height:1.7;margin:0 0 12px;">Bonjour ${nom},</p>
    <p style="color:#5C7370;line-height:1.7;margin:0 0 12px;">
      Nous avons bien reçu votre demande pour <strong style="color:#0E8C8C;">${typeEvenement}</strong>${date ? `, envisagé le <strong style="color:#16302E;">${dateFormatted}</strong>` : ""}.
    </p>
  ...
  try {
    await sendEmail(apiKey, {
      ...
      subject: `📋 Nouvelle demande — ${typeEvenement} — ${nom}`,
    ...
    console.error("demande-contact:", err.message);
    return res.status(500).json({ error: "Échec de l'envoi. Veuillez réessayer ou nous écrire directement à info@wentenevenementielreunion.com" });
  }
}
