...
const row = (label, value, alt) =>
  `<tr${alt ? ' style="background:#F4FBFA"' : ""}><td style="padding:9px 12px;font-weight:600;color:#5C7370;width:200px;font-size:13px;">${label}</td><td style="padding:9px 12px;font-size:14px;color:#16302E;">${value}</td></tr>`;
...
export default async function handler(req, res) {
  ...
  if (!prenom || !nom || !email || !telephone || !periode || !nombrePersonnes) {
...
<div style="font-family:'Inter',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #E2EFEC;">
  <div style="background:#0E8C8C;padding:24px 28px;">
    <p style="color:#fff;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;margin:0 0 6px;">Wenten Événementiel — Nouvelle demande séjour</p>
    <h1 style="color:#fff;font-family:Georgia,serif;font-size:24px;font-weight:400;margin:0;">🌴 SÉJOUR MAURICE COMPLET — Lagoon Love</h1>
  </div>
  <div style="padding:28px;">
    <table style="border-collapse:collapse;width:100%;border-radius:8px;overflow:hidden;border:1px solid #E2EFEC;">
      ${row("Prénom", prenom, false)}
      ${row("Nom", nom, true)}
      ${row("Email", `<a href="mailto:${email}" style="color:#0E8C8C;">${email}</a>`, false)}
      ${row("Téléphone", telephone, true)}
      ${row("Période souhaitée", periode, false)}
      ${row("Nombre de personnes", nombrePersonnes, true)}
      ${row("Budget approximatif", budget || "Non renseigné", false)}
...
<div style="font-family:'Inter',Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;padding:0;border-radius:16px;overflow:hidden;border:1px solid #E2EFEC;">
  <div style="background:#F4FBFA;padding:28px 32px 24px;border-bottom:1px solid #E2EFEC;">
    ...
    <p style="font-size:11px;color:#5C7370;letter-spacing:0.08em;text-transform:uppercase;margin:0;">La Réunion · Depuis 2012</p>
  </div>
  <div style="padding:32px;">
    <h2 style="font-family:Georgia,serif;font-size:24px;font-weight:400;color:#16302E;margin:0 0 20px;">Votre demande de séjour à l'Île Maurice 🌴</h2>
    ...
    <p style="color:#5C7370;line-height:1.7;margin:0 0 12px;">
      Nous avons bien reçu votre demande pour un <strong style="color:#0E8C8C;">séjour complet à l'Île Maurice</strong> incluant le scénario Lagoon Love, pour la période <strong style="color:#16302E;">${periode}</strong>.
    </p>
    <p style="color:#5C7370;line-height:1.7;margin:0 0 24px;">
      Notre équipe vous recontacte très prochainement pour construire votre séjour sur mesure.
    </p>
  ...
  try {
    ...
    await sendEmail(apiKey, {
      ...
      subject: "Votre demande de séjour à l'Île Maurice — Wenten Événementiel",
      htmlContent: clientHtml,
    });
    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error("demande-sejour-maurice:", err.message);
    return res.status(500).json({ error: "Échec de l'envoi. Veuillez réessayer ou nous contacter directement." });
  }
}
